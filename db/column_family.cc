...
WriteStallCondition ColumnFamilyData::RecalculateWriteStallConditions() {
  ...
  if (current_ != nullptr) {
	...
	/* add by yanrui start*/
	if (mutable_cf_options_.auto_tuned_memtable) {
	  mutable_cf_options_.auto_tuned_memtable = env_->auto_tuned_memtable;
	  mutable_cf_options_.write_buffer_size = env_->write_buffer_size;
	  mutable_cf_options_.max_write_buffer_number = env_->max_write_buffer_number;
	}
    
	ROCKS_LOG_INFO(
          ioptions_.info_log,
          "RecalculateWriteStallConditions::write_buffer_size %d",
          mutable_cf_options_.write_buffer_size);
	ROCKS_LOG_INFO(
          ioptions_.info_log,
          "RecalculateWriteStallConditions max_write_buffer_number %d",
	...
	DBOptions options;
	uint64_t default_max_write_buffer_number_ = options.default_max_write_buffer_number;
	ROCKS_LOG_INFO(
          ioptions_.info_log,
          "RecalculateWriteStallConditions::default_max_write_buffer_number is set to %d",
          mutable_cf_options.default_max_write_buffer_number);
    
	/* 定义小于等于一半为远远小于 */
	/* */	  
	if(1 == 0){
	    uint64_t max_write_buffer_number_ = mutable_cf_options.max_write_buffer_number;
		if (imm()->NumNotFlushed() == max_write_buffer_number_-1) {
			max_write_buffer_number_++;
			mutable_cf_options_.max_write_buffer_number = max_write_buffer_number_;
			ROCKS_LOG_WARN(
	          ioptions_.info_log,
	          "[%s] Nearly Stopping writes because we have %d immutable memtables "
	          "(waiting for flush), max_write_buffer_number is adjust to %d",
	          name_.c_str(), imm()->NumNotFlushed(),
	          max_write_buffer_number_);
		}else if(imm()->NumNotFlushed() <= max_write_buffer_number_ / 2 && max_write_buffer_number_ > default_max_write_buffer_number_){
			max_write_buffer_number_--;
			mutable_cf_options_.max_write_buffer_number = max_write_buffer_number_;
			ROCKS_LOG_WARN(
	          ioptions_.info_log,
	          "[%s] Resume writes because we have %d immutable memtables "
	          "(waiting for flush), max_write_buffer_number is adjust to %d",
	          name_.c_str(), imm()->NumNotFlushed(),
	          max_write_buffer_number_);
		}
...
MemTable* ColumnFamilyData::ConstructNewMemtable(
  ...
  return new MemTable(internal_comparator_, ioptions_, mutable_cf_options,
                      write_buffer_manager_, earliest_seq, id_, env_);
}
