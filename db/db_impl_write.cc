...
Status DBImpl::SwitchMemtable(ColumnFamilyData* cfd, WriteContext* context) {
  ...
  /*add by yanrui start*/
  // ImmutableCFOptions ioptions_ = *cfd->ioptions();
  MutableCFOptions curr_options = *cfd->GetLatestMutableCFOptions();
  /* 获取最新的值*/
 // env_->write_buffer_size =  curr_options.write_buffer_size;
 // env_->max_write_buffer_number =  curr_options.max_write_buffer_number;
  ROCKS_LOG_INFO(immutable_db_options_.info_log, "the Latest write_buffer_size %d\n",curr_options.write_buffer_size);
  ROCKS_LOG_INFO(immutable_db_options_.info_log, "the Latest max_write_buffer_number %d\n",curr_options.max_write_buffer_number);
	  
  /* 获取这个memtable开关的值 */
  bool auto_tuned_memtable_ = curr_options.auto_tuned_memtable;
  //bool auto_tuned_memtable_ = cfd->GetLatestMutableCFOptions()->auto_tuned_memtable;
  //uint64_t write_buffer_size_ = curr_options.write_buffer_size;
  //ROCKS_LOG_INFO(immutable_db_options_.info_log, "the MutableCFOptions write_buffer_size %d\n",write_buffer_size_); 
  
  ROCKS_LOG_INFO(immutable_db_options_.info_log, "the disable_auto_memtable_ %d\n",auto_tuned_memtable_);
  //  ROCKS_LOG_INFO(immutable_db_options_.info_log, "the env disable_auto_memtable_ %d\n",env_->auto_tuned_memtable);
  /* 记录 memtable 数据量达到容量所需要的时间t,即1次转换的时间,即为平均写入速度 */
  // uint64_t write_buffer_size_ = env_->write_buffer_size;
  //ROCKS_LOG_INFO(immutable_db_options_.info_log, "the default_write_buffer_size_ %d\n",default_write_buffer_size_);

  /* 获取未flush的数量     */
  int num_imm_unflushed = cfd->imm()->NumNotFlushed();
  if(auto_tuned_memtable_){   /*打开了自动调整memtable的开关         */
	  switch_memtable_count.Increment();
	  uint64_t count_ = switch_memtable_count.Read();
	  ROCKS_LOG_INFO(immutable_db_options_.info_log,"call the switch counts %" PRIu64 "\n",count_);
	  uint64_t interval = 0;
	  double raito = 0.0;
	  // uint64_t standard = 0;
	  /* 统计switch的频率*/
	  /* 每10次switch记录一下时间，调整一次 buffer_size */
	  if(count_ % 10 == 1){
	        start_time_ =  env_->NowMicros();
			/*ROCKS_LOG_INFO(immutable_db_options_.info_log,"the start time %" PRIu64 "\n",start_time_);*/
	  }else if(count_ % 10 == 0){
	        current_time_ =  env_->NowMicros();
			/*ROCKS_LOG_INFO(immutable_db_options_.info_log,"the current time %" PRIu64 "\n",current_time_);*/
			interval = current_time_ - start_time_ ;   //(10次 微秒)
			double v_current_ = (interval / 1000000.0) / 10;  //(memtable数据的平均写入速度            次/s)
			/*
			ROCKS_LOG_INFO(immutable_db_options_.info_log,"the interval time %" PRIu64 "\n",interval);
		    ROCKS_LOG_INFO(immutable_db_options_.info_log,"the v_prev_ %lf\n",v_prev_);
		    ROCKS_LOG_INFO(immutable_db_options_.info_log, "the v_current_ %lf\n", v_current_);
			*/			 
			if(v_prev_ > 0.0){			   
				raito = v_current_ / v_prev_ ;ROCKS_LOG_INFO(immutable_db_options_.info_log,"the v_current_/v_pre_ raito %lf\n",raito);
				/* 调整参数
					write_buffer_size
					max_write_buffer_number
				*/
				env_->TunedMemtable(raito,num_imm_unflushed);
			}else{
				// Nothing to do
			}
			// 	写入速率更新 
	  ...
	  // 更新    MutableCFOptions 
	  //curr_options.write_buffer_size = env_->write_buffer_size;
	  //curr_options.max_write_buffer_number = env_->max_write_buffer_number;
	  //const MutableCFOptions &mumtable_options_ = curr_options;
	  
	  ROCKS_LOG_INFO(immutable_db_options_.info_log,"the pre_write_buffer_size %d\n",env_->write_buffer_size);
	  ROCKS_LOG_INFO(immutable_db_options_.info_log,"the default_max_write_buffer_number %d\n",env_->max_write_buffer_number);
	  
	  //ROCKS_LOG_INFO(immutable_db_options_.info_log,
		//							"the write_buffer_size %d\n",
		//							mumtable_options_.write_buffer_size);
	  //ROCKS_LOG_INFO(immutable_db_options_.info_log,
		//							"the max_write_buffer_number %d\n",
		//							mumtable_options_.max_write_buffer_number);
	  // 更新到options
	  //curr_options.RefreshDerivedOptions(ioptions_);
	  //curr_options.set_write_buffer_size();
  }
  ...
  /* add by yanrui  move int num_imm_unflushed = cfd->imm()->NumNotFlushed(); */
  //int num_imm_unflushed = cfd->imm()->NumNotFlushed();
