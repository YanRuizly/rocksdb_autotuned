...
void DBImpl::BloomFilterAuto(double getmicrosec_rate){
	/* 传入 读频率 */
	...
	ROCKS_LOG_INFO(ioptions.info_log,
		"XXXXXXXXXXXXX prefix_extractor_ %" PRIu64 "\n",
	     ioptions.prefix_extractor ? 1:0);
	     */
};
...
Status DBImpl::GetImpl(const ReadOptions& read_options,
  ...
  /* add by yanrui start */ 
  /* 设置 auto_bloom _filter 的开关打开  应该是通过db_bench 修改这个env*/
  env_->auto_tuned_bloom_filter = true;
  
  if(env_->auto_tuned_bloom_filter){
	  search_count.Increment();
	  int count_ = search_count.Read();
	  int interval = 0; /* 每100次查询的时间差 */	 
	  /* 标准读频率       设置到不可变变量中 */
	  // int standard_getmicrosec = 164;  // 0.25次/微秒       (interval时间间隔 ms / 10   0 10次40微秒)

	  /* 与标准的频率比率 */
	  double getmicrosec_ratio = 0.0;
	  /*
	  ROCKS_LOG_INFO(immutable_db_options_.info_log,
			"XXXXXXXXXXXXX call the search counts %" PRIu64 "\n",
		     count_);
	  */
	  if(count_ % 100 ==  1){
		        search_start_time_ = env_->NowMicros();
				/*
				ROCKS_LOG_INFO(immutable_db_options_.info_log,
									"the start time %" PRIu64 "\n",
									search_start_time_);
									*/
	  }else if(count_ % 100 == 0){
		        search_current_time_ =  env_->NowMicros();
				/*
				ROCKS_LOG_INFO(immutable_db_options_.info_log,
		    	                   "the current time %" PRIu64 "\n",
		        	               search_current_time_);
		        	               */
				interval = ( search_current_time_ - search_start_time_ ) ; //(s)

				ROCKS_LOG_INFO(immutable_db_options_.info_log,
		    	                   "the interval time %" PRIu64 "\n",
		        	               interval);

				/* 确定标准值，计算前1000次读请求的平均值*/
			    if(count_ <= 1000){
				  	 search_interval_ += interval;
				}else{
					 standard_getmicrosec_ = search_interval_ / 11;
					 getmicrosec_ratio = interval / ( standard_getmicrosec_  * 1.0 );
					 ROCKS_LOG_INFO(immutable_db_options_.info_log,"XXXXXXXXXXXXX standard_getmicrosec_ %d\n",search_interval_);
					
					 ROCKS_LOG_INFO(immutable_db_options_.info_log,"XXXXXXXXXXXXX getmicrosec_ratio %lf\n",getmicrosec_ratio);
					 if(getmicrosec_ratio >= 1.0 || getmicrosec_ratio <= 0.75){
					 	env_->TunedBloomFiler(getmicrosec_ratio);
						ROCKS_LOG_INFO(immutable_db_options_.info_log,"XXXXXXXX memtable_prefix_bloom_size_ratio %lf\n",env_->memtable_prefix_bloom_size_ratio);
					 }
				}	
				//ROCKS_LOG_INFO(immutable_db_options_.info_log,"XXXXXXXXXXXXX bloom_filter_size %d \n",env_->bloom_filter_size);
	  }	
  }
  /* add by yanrui end */
